...
### Beginner reading and explanations

The direct reader now leads with short preparation guidance and a large Say these words transliteration (25/39), then primary-language translation and Hebrew. Practical directions are separate from recitable text. After-reading steps follow the last passage; source links remain available. English fallback translations are labeled honestly. Excerpts and service overviews have explicit scope notes rather than appearing to be complete prayers.

Sourced guidance currently covers the bundled Modeh Ani, Asher Yatzar, Hamotzi, Shema, tallit blessing and arm-tefillin blessing, plus prayer-book sections explicitly titled Amidah. Other entries retain their existing purpose descriptions; this is not comprehensive ritual instruction or a rabbinically approved edition. Do not infer detailed ritual steps from fuzzy title matches. The sources are linked in src/data/prayerReadingGuide.ts, including Chabad's waking, handwashing, bread, Shema, tallit, tefillin and Amidah guides. Customs and incomplete texts are identified where relevant.

Explain this prayer / Explain this passage expands from the tapped control into a full-screen detail using measured bounds and core Animated. Reduced Motion removes the transition. The explanation uses the existing live streaming AI service, asks for two short sentences (up to 45 words) in the primary language, and explicitly excludes ritual instruction. Existing AI consent is respected, errors offer retry, and closing restores the reader. The supplied Reacticx snippet describes usage but does not include its implementation; this equivalent interaction requires no map or new native dependencies.

Validation: typecheck, targeted lint, three scope/guidance tests and three existing reader interaction tests passed. Live API returned HTTP 200 with a short Modeh Ani explanation. Native iPhone 17 Pro screenshots verified pronunciation hierarchy and the expanded consent and live-answer views. Temporary auto-open and public-text QA consent overrides were removed without changing the saved preference or logging prayer activity. Touch-based testing remains limited by the desktop session.

### One conversation in the prayer reader

Ask about this prayer remains visible in the reader header and opens In simple words with a question composer. Opening alone sends nothing. Explain simply is an optional starter inside the conversation; multi-passage Explain this passage shortcuts use the same provider and transcript. The former assistant panel and its separate consent/state flow have been removed from Source & options.

Follow-up requests include recent user/assistant turns plus bounded prayer/source context. Failed answers are excluded from future context. Closing the conversation preserves messages and unsent text while the reader remains open; starting a different prayer resets the conversation. Conversations are in-memory, not a permanent chat archive. Only one request can run at once; errors can retry without duplicating the user turn. The input uses keyboard avoidance and multiline expansion; responses auto-scroll only while already near the bottom. AI consent is retained and queued questions wait for explicit acceptance.

Verification: two context tests, three reader tests and a conversation interaction test cover follow-up context, no request on open, and draft/history retention across close/reopen. Typecheck and targeted production lint pass. Native iPhone 17 Pro screenshots show the persistent entry point and expanded composer/consent state. Temporary preview auto-open overrides were removed. No new native dependencies or additional Metro session.
