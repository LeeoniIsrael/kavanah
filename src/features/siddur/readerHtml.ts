...
export function createReaderHtml(
 ...
 let x=0,y=0,t=0,pinching=false,dragging=false,lastDrag=0;document.addEventListener('touchstart',e=>{if(e.touches.length>1){pinching=true;send({type:'pinchStart'});return}x=e.touches[0].clientX;y=e.touches[0].clientY;t=Date.now();dragging=false},{passive:true});document.addEventListener('touchmove',e=>{if(pinching||e.touches.length!==1||window.getSelection()?.toString())return;let dx=e.touches[0].clientX-x,dy=e.touches[0].clientY-y;if(Math.abs(dx)>8&&Math.abs(dx)>Math.abs(dy)*1.4&&Date.now()-lastDrag>30){dragging=true;lastDrag=Date.now();send({type:'drag',dx})}},{passive:true});document.addEventListener('touchend',e=>{if(pinching){pinching=false;send({type:'pinchEnd'});return}if(window.getSelection()?.toString()){if(dragging)send({type:'dragEnd'});return}let dx=e.changedTouches[0].clientX-x,dy=e.changedTouches[0].clientY-y;if(Math.abs(dx)>Math.abs(dy)*1.4&&((Math.abs(dx)>26&&Date.now()-t<650)||Math.abs(dx)>innerWidth*.18)){send({type:'swipe',dx});return}if(dragging)send({type:'dragEnd'});if(Math.abs(dx)<8&&Math.abs(dy)<8&&Date.now()-t<300){let side=x/innerWidth;if(side<.2||side>.8)send({type:'edge',side})}},{passive:true});document.addEventListener('touchcancel',()=>{if(dragging)send({type:'dragEnd'});dragging=false},{passive:true});
