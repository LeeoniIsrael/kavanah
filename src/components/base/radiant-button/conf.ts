export const BORDER_GLOW_SHADER = `
  ...
  half4 main(float2 fragCoord) {
    ...
    float borderMask = smoothstep(0.0, 1.5, distInner) *
      (1.0 - smoothstep(-1.5, 0.0, distOuter));
    ...
    float perimeterProgress = (angle + 3.14159265) / 6.28318530;
    float dist = perimeterProgress - progress;
    ...
    float alpha = max(glow, 0.15) * borderMask;
