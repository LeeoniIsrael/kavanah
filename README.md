...
The first launch shows a short animated welcome and keeps local exploration available. When `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY` are set in a private `.env`, email-code sign-in appears. Its OTP email template must contain `{{ .Token }}`; a custom SMTP service is needed before sending to users outside the Supabase team. Apple, Google, and phone buttons stay hidden until their provider setup is verified and the matching `EXPO_PUBLIC_ENABLE_*_SIGN_IN` flag is set to `true`. Google also needs `kavanah://auth` in Auth redirect settings; phone needs an SMS provider. Never put provider secrets in Expo variables.
